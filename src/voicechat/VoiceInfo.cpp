...
VoiceInfo::VoiceInfo()
 : m_format(), m_maxAmplitude( 0 )
{
  setAudioFormat( AudioManager::instance().defaultAudioFormat() );
}

bool VoiceInfo::setAudioFormat( const QAudioFormat& audio_format )
{
...
qreal VoiceInfo::processData( const char* data, int data_size )
{
  if( !m_format.isValid() )
    return qreal( 0 );
  if( m_format.sampleSize() % 8 != 0 )
    qWarning() << "Invalid audio format (sample size) found in VoiceInfo::processData(...)";
  ...
  if( data_size % sample_bytes != 0 )
    qWarning() << "Invalid audio format (sample bytes and data size) found in VoiceInfo::processData(...)";
  ...
  return qreal( max_value ) / m_maxAmplitude;
}
